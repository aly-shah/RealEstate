  ...
  return (
    <div className="mb-8 space-y-5">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Current liability" value={money(data.liability)} tone="danger" sub="Approved, not yet paid" />
        <StatCard label="Weighted forecast" value={money(data.weightedTotal)} tone="accent" sub="Probability-adjusted" />
        <StatCard label="Gross pipeline" value={money(data.grossPipeline)} tone="ink" sub="If every deal closed" />
...
export function AgentForecastCards({ data }: { data: AgentForecast }) {
  return (
    <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
      <StatCard label="Paid to you" value={money(data.paid)} tone="ok" />
      <StatCard label="Pending" value={money(data.pending)} tone="gold" sub="Approved, awaiting payout" />
      <StatCard
        label="Forecast"
