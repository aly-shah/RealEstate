...
/** Row controls for a project unit — quick-edit price/status, or delete. */
export function UnitActions({ unit }: { unit: Unit }) {
  ...
  return (
    <div className="flex items-center justify-end gap-1.5">
      <button type="button" onClick={() => { setPrice(String(unit.salePrice || "")); setStatus(unit.status); setError(null); setOpen(true); }} className="btn-ghost !px-2 !py-1 text-xs">Edit</button>
      ...
      <Drawer open={open} onClose={() => setOpen(false)} title={`Edit ${unit.reference}`} description="Adjust this unit's price or status." width="sm">
        <div className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button type="button" onClick={save} disabled={saving} className="btn-accent">{saving ? "Saving…" : "Save"}</button>
          </div>
        </div>
      </Drawer>
    </div>
